...
void GenerateProblem(int npx, int npy, int npz, local_int_t nx, local_int_t ny, local_int_t nz, int size, int rank, Problem *problem){
    ...
    GhostCell tmp_injection_ghost_cells[NUMBER_NEIGHBORS] = {
        ...
        // FRONT_NE (corner injection): corresponds to hipMemcpy(x_d + dimx - 1, ...),
        // which gives coordinate (dimx - 1, 0, 0)
        { dimx - 1, 0, 0,    dimx, dimy, dimz,    1, 1, 1 },

        // FRONT_SE (corner injection): corresponds to hipMemcpy(x_d + dimx * dimy - 1, ...),
        // i.e. (dimx - 1, dimy - 1, 0)
        { dimx - 1, dimy - 1, 0,    dimx, dimy, dimz,    1, 1, 1 },

        // FRONT_SW (corner injection): corresponds to hipMemcpy(x_d + (dimy - 1) * dimx, ...),
        // i.e. (0, dimy - 1, 0)
        { 0, dimy - 1, 0,    dimx, dimy, dimz,    1, 1, 1 },

        // FRONT_NW (corner injection): corresponds to hipMemcpy(x_d, ...),
        // i.e. (0, 0, 0)
        { 0, 0, 0,    dimx, dimy, dimz,    1, 1, 1 },

        // BACK_NE (corner injection): corresponds to hipMemcpy(x_d + dimx - 1 + dimx * dimy * (dimz - 1), ...),
        // i.e. (dimx - 1, 0, dimz - 1)
        { dimx - 1, 0, dimz - 1,    dimx, dimy, dimz,    1, 1, 1 },

        // BACK_SE (corner injection): corresponds to hipMemcpy(x_d + dimx - 1 + (dimy - 1) * dimx + dimx * dimy * (dimz - 1), ...),
        // i.e. (dimx - 1, dimy - 1, dimz - 1)
        { dimx - 1, dimy - 1, dimz - 1,    dimx, dimy, dimz,    1, 1, 1 },

        // BACK_SW (corner injection): corresponds to hipMemcpy(x_d + dimx * (dimy - 1) + dimx * dimy * (dimz - 1), ...),
        // i.e. (0, dimy - 1, dimz - 1)
        { 0, dimy - 1, dimz - 1,    dimx, dimy, dimz,    1, 1, 1 },

        // BACK_NW (corner injection): corresponds to hipMemcpy(x_d + dimx * dimy * (dimz - 1), ...),
        // i.e. (0, 0, dimz - 1)
...
void InitHaloMemGPU(Halo *halo, Problem *problem){
    ...
    DataType *x_d;
    CHECK_CUDA(hipMalloc(&x_d, dimx * dimy * dimz * sizeof(DataType)));
    CHECK_CUDA(hipMemset(x_d, 0, dimx * dimy * dimz * sizeof(DataType)));
    halo->x_d = x_d;
    ...
    for(int i = 0; i < NUMBER_NEIGHBORS; i++){
        CHECK_CUDA(hipMalloc(&(halo->send_buff_d[i]), problem->count_exchange[i] * sizeof(DataType)));
        CHECK_CUDA(hipMemset(halo->send_buff_d[i], 0, problem->count_exchange[i] * sizeof(DataType)));
        CHECK_CUDA(hipMalloc(&(halo->recv_buff_d[i]), problem->count_exchange[i] * sizeof(DataType)));
        CHECK_CUDA(hipMemset(halo->recv_buff_d[i], 0, problem->count_exchange[i] * sizeof(DataType)));
...
void SetHaloZeroGPU(Halo *halo){
    CHECK_CUDA(hipMemset(halo->x_d, 0, halo->dimx * halo->dimy * halo->dimz * sizeof(DataType)));
}
...
void SetHaloGlobalIndexGPU(Halo *halo, Problem *problem){
    ...
    CHECK_CUDA(hipMemcpy(halo->x_d, x_h, halo->dimx * halo->dimy * halo->dimz * sizeof(DataType), hipMemcpyHostToDevice));
}
...
void FreeHaloGPU(Halo *halo){
    CHECK_CUDA(hipFree(halo->x_d));
    for(int i = 0; i<NUMBER_NEIGHBORS; i++){
            CHECK_CUDA(hipFree(halo->send_buff_d[i]));
            CHECK_CUDA(hipFree(halo->recv_buff_d[i]));
    }
}
...
void InitGPU(Problem *problem){
    ...
    CHECK_CUDA(hipSetDevice(problem->rank % deviceCount));
...
void extract_horizontal_plane_from_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff){
    ...
    if(host_buff) {
        CHECK_CUDA(hipMemcpy(halo->send_buff_h[i_buff], buff, gh->length_X*gh->length_Z*sizeof(DataType), hipMemcpyDeviceToHost));
    }
}
...
void inject_horizontal_plane_to_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff){
    ...
    if (host_buff){
        CHECK_CUDA(hipMemcpy(buff, halo->recv_buff_h[i_buff], gh->length_X*gh->length_Z*sizeof(DataType), hipMemcpyHostToDevice));
    }
...
void extract_vertical_plane_from_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff){
    ...
    // copy from device to host
    if (host_buff){
        CHECK_CUDA(hipMemcpy(halo->send_buff_h[i_buff], buff, gh->length_Y*gh->length_Z*sizeof(DataType), hipMemcpyDeviceToHost));
    }
}
...
void inject_vertical_plane_to_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff){
    ...
    DataType *buff = halo->recv_buff_d[i_buff];
    if(host_buff){
        CHECK_CUDA(hipMemcpy(buff, halo->recv_buff_h[i_buff], gh->length_Y*gh->length_Z*sizeof(DataType), hipMemcpyHostToDevice));
    }
...
void extract_frontal_plane_from_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff){
    ...
    // copy from device to host
    CHECK_CUDA(hipMemcpy(halo->send_buff_h[i_buff], buff, gh->length_X*gh->length_Y*sizeof(DataType), hipMemcpyDeviceToHost));
}
...
void inject_frontal_plane_to_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff){
    ...
    DataType *buff = halo->recv_buff_d[i_buff];
    if(host_buff){
        CHECK_CUDA(hipMemcpy(buff, halo->recv_buff_h[i_buff], gh->length_Y*gh->length_X*sizeof(DataType), hipMemcpyHostToDevice));
    }
...
void extract_edge_X_from_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff) {
    ...
    if(host_buff){
        CHECK_CUDA(hipMemcpy(halo->send_buff_h[i_buff], buff, gh->length_X * sizeof(DataType), hipMemcpyDeviceToHost));
    }
}
...
void inject_edge_X_to_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff) {
    ...
    DataType *buff = halo->recv_buff_d[i_buff];
    if(host_buff){
        CHECK_CUDA(hipMemcpy(buff, halo->recv_buff_h[i_buff], gh->length_X * sizeof(DataType), hipMemcpyHostToDevice));
    }
...
void extract_edge_Y_from_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff){
    ...
    if(host_buff){
        CHECK_CUDA(hipMemcpy(halo->send_buff_h[i_buff], buff, gh->length_Y * sizeof(DataType), hipMemcpyDeviceToHost));
    }
}
...
void inject_edge_Y_to_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff) {
    ...
    DataType *buff = halo->recv_buff_d[i_buff];
    if(host_buff){
        CHECK_CUDA(hipMemcpy(buff, halo->recv_buff_h[i_buff], gh->length_Y * sizeof(DataType), hipMemcpyHostToDevice));
    }
...
void extract_edge_Z_from_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff){
    ...
    if(host_buff){
        CHECK_CUDA(hipMemcpy(halo->send_buff_h[i_buff], buff, gh->length_Z * sizeof(DataType), hipMemcpyDeviceToHost));
    }
}
...
void inject_edge_Z_to_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff) {
    ...
    DataType *buff = halo->recv_buff_d[i_buff];
    if (host_buff){
        CHECK_CUDA(hipMemcpy(buff, halo->recv_buff_h[i_buff], gh->length_Z * sizeof(DataType), hipMemcpyHostToDevice));
    }
...
void extract_corner_from_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff) {
    ...
    for (int z = 0; z < gh->length_Z; z++) {
        for (int y = 0; y < gh->length_Y; y++) {
            ...
            CHECK_CUDA(hipMemcpy(buff, x_d, gh->length_X * sizeof(DataType), hipMemcpyDeviceToHost));
            buff += gh->length_X;
    ...
    if(!host_buff){
        CHECK_CUDA(hipMemcpy(halo->send_buff_d[i_buff], buff, gh->length_X*gh->length_Y*gh->length_Z * sizeof(DataType), hipMemcpyHostToDevice));
    }
}
...
void inject_corner_to_GPU(Halo *halo, int i_buff, GhostCell *gh, bool host_buff) {
    ...
    for (int z = 0; z < gh->length_Z; z++) {
        for (int y = 0; y < gh->length_Y; y++) {
            // Copy a contiguous block of length_X elements (one row of the corner)
            CHECK_CUDA(hipMemcpy(x_d, buff, gh->length_X * sizeof(DataType), hipMemcpyHostToDevice));
            buff += gh->length_X;
            x_d += gh->dimx;  // move to the next row in the halo
        }
        // Jump to the start of the next z-plane:
    ...
    if(!host_buff){
        CHECK_CUDA(hipMemcpy(halo->recv_buff_d[i_buff], buff, gh->length_X*gh->length_Y*gh->length_Z * sizeof(DataType), hipMemcpyHostToDevice));
    }
}
...
void SendResult(int rank_recv, Halo *x_d, Problem *problem){
    ...
    for(int i = 0; i < problem->nz; i++){
        for(int j = 0; j<problem->ny; j++){
            CHECK_CUDA(hipMemcpy(send_buf_h, send_addr_d, problem->nx * sizeof(DataType), hipMemcpyDeviceToHost));
        ...
        for(int j = 0; j<problem->gny; j++){ // go through all gny rows
            ...
            for(int l = 0; l<problem->npx; l++){ // go thorugh all nx columns
                ...
                if(px_recv == problem->px && py_recv == problem->py && pz_recv == problem->pz){ //gathering rank holds the data
                    CHECK_CUDA(hipMemcpy(result_h, own_data_d, problem->nx * sizeof(DataType), hipMemcpyDeviceToHost));
...
void PrintHalo(Halo *x_d){
    ...
    CHECK_CUDA(hipMemcpy(x_h, x_d->x_d, x_d->dimx * x_d->dimy * x_d->dimz * sizeof(DataType), hipMemcpyDeviceToHost));

    ...
    for (local_int_t i = coop_group_id; i < num_nodes_with_color; i += num_coop_groups){
        ...
        for(int stripe = lane; stripe < num_stripes; stripe += cooperation_number){
            ...
            if (gj>= 0 && gj < gnx * gny * gnz) {
                ...
                if(hj>=0 && hj<(nx+2)*(ny+2)*(nz+2))my_sum -= striped_A[li * num_stripes + stripe] * x[hj];
            }
        }
...
void striped_multi_GPU_Implementation<T>::striped_box_coloring_multi_GPU_computeSymGS(
    ...
    //if(problem->rank == 0) printf("MULTI bx: %d by: %d bz: %d\n", bx, by, bz);
    ...
    */
    ...
    local_int_t num_blocks = std::min(ceiling_division(max_num_rows_per_color, 1024/cooperation_number), MAX_NUM_BLOCKS);
    /*
    double L2_norm_y;
    ...
    */
    ...
    for(int color = 0; color <= max_color; color++){
            ...
            striped_box_coloring_half_SymGS_kernel<<<num_blocks, 512>>>(
            cooperation_number,
            color, bx, by, bz,
            nx, ny, nz,
            num_rows, num_cols,

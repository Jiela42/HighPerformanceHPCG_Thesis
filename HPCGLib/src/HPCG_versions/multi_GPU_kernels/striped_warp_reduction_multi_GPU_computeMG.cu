// This file implements the Multigrid (MG) V-cycle on multiple GPUs using striped storage,
// including pre-smoothing, restriction, coarse solve, prolongation, and post-smoothing.
...
/**
 * @brief Executes a Multigrid (MG) V-cycle to solve Ax = r using recursive calls on coarser levels.
 *
 * Performs pre-smoothing using SymGS, restricts residuals, recursively solves coarse-level systems,
 * applies prolongation, and finishes with post-smoothing. Works on halo-extended data and uses GPU kernels
 * for restriction and prolongation.
 *
 * @param A        Matrix with multigrid hierarchy information.
 * @param r_d      RHS vector (device, halo-aware).
 * @param x_d      Solution vector (device, halo-aware), updated in place.
...
void striped_multi_GPU_Implementation<T>::striped_warp_reduction_multi_GPU_computeMG(
    ...
    if (A.get_coarse_Matrix() != nullptr) { // go to coarser level if it exists
        ...
        // Coarse grid problem setup and recursive MG call
        Problem coarse_problem;
        GenerateProblem(problem->npx, problem->npy, problem->npz,
                        problem->nx / 2, problem->ny / 2, problem->nz / 2,
                        problem->size, problem->rank, &coarse_problem);

...
Problem* NCCL_Implementation<T>::init_comm_NCCL(int argc, char *argv[], int npx, int npy, int npz, int nx, int ny, int nz) {
    ...
    CHECK_CUDA(hipSetDevice(localRank));
    CHECK_CUDA(hipStreamCreate(&this->hip_stream));
...
void NCCL_Implementation<T>::ExchangeHaloNCCL(Halo *halo, Problem *problem) {
    ...
    for(int i = 0; i<NUMBER_NEIGHBORS; i++){
        CHECK_NCCL(ncclRecv(halo->recv_buff_d[i], problem->count_exchange[i], NCCL_TYPE,
                    problem->neighbors[i], this->nccl_comm, hip_stream));
    ...
    CHECK_CUDA(hipStreamSynchronize(this->hip_stream));

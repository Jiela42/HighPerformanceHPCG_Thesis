...
std::vector<local_int_t> color_for_forward_pass(striped_Matrix <DataType>& A){
    ...
    CHECK_CUDA(hipMalloc(&colors_d, num_rows * sizeof(local_int_t)));
    CHECK_CUDA(hipMemset(colors_d, -1, num_rows * sizeof(local_int_t)));
    ...
    CHECK_CUDA(hipMemcpy(colors.data(), colors_d, num_rows * sizeof(local_int_t), hipMemcpyDeviceToHost));
...
std::vector <local_int_t> color_for_backward_pass(striped_Matrix <DataType>& A){
    ...
    CHECK_CUDA(hipMalloc(&colors_d, num_rows * sizeof(local_int_t)));
    CHECK_CUDA(hipMalloc(&j_min_i_d, num_rows * sizeof(local_int_t)));
    CHECK_CUDA(hipMalloc(&A_d, num_stripes * num_rows * sizeof(DataType)));

    CHECK_CUDA(hipMemset(colors_d, -1, num_rows * sizeof(local_int_t)));
    CHECK_CUDA(hipMemcpy(j_min_i_d, A.get_j_min_i().data(), num_rows * sizeof(local_int_t), hipMemcpyHostToDevice));
    CHECK_CUDA(hipMemcpy(A_d, A.get_values().data(), num_stripes * num_rows * sizeof(DataType), hipMemcpyHostToDevice));
...
std::pair<std::vector<local_int_t>, std::vector<local_int_t>> get_color_row_mapping(int nx, int ny, int nz)
{
    ...
    CHECK_CUDA(hipMalloc(&color_pointer_d, (max_color+1) * sizeof(local_int_t)));
    CHECK_CUDA(hipMalloc(&color_sorted_rows_d, num_rows * sizeof(local_int_t)));

    CHECK_CUDA(hipMemset(color_pointer_d, 0, (max_color+1) * sizeof(local_int_t)));

    get_color_row_mapping(nx, ny, nz, color_pointer_d, color_sorted_rows_d);

    // copy the results back to the host
    CHECK_CUDA(hipMemcpy(color_pointer.data(), color_pointer_d, (max_color+1) * sizeof(local_int_t), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(color_sorted_rows.data(), color_sorted_rows_d, num_rows * sizeof(local_int_t), hipMemcpyDeviceToHost));
...
void get_color_row_mapping_for_boxColoring(
    ...
    CHECK_CUDA(hipMemcpy(color_pointer_d, color_ptr_h.data(), (num_colors+1) * sizeof(local_int_t), hipMemcpyHostToDevice));

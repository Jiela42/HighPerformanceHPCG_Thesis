...
sparse_CSR_Matrix<T>::~sparse_CSR_Matrix(){
    ...
    if (this->row_ptr_d != nullptr) {
        ...
        CHECK_CUDA(hipFree(this->row_ptr_d));
        // std::cout << "row_ptr_d freed" << std::endl;
        this->row_ptr_d = nullptr;
    }
    if (this->col_idx_d != nullptr) {
        CHECK_CUDA(hipFree(this->col_idx_d));
        this->col_idx_d = nullptr;
    }
    if (this->values_d != nullptr) {
        CHECK_CUDA(hipFree(this->values_d));
        this->values_d = nullptr;
    }
    if(this->f2c_op_d != nullptr){
        CHECK_CUDA(hipFree(this->f2c_op_d));
        this->f2c_op_d = nullptr;
    }
    if(this->rc_d != nullptr){
        CHECK_CUDA(hipFree(this->rc_d));
        this->rc_d = nullptr;
    }
    if(this->xc_d != nullptr){
        CHECK_CUDA(hipFree(this->xc_d));
        this->xc_d = nullptr;
    }
    if(this->Axf_d != nullptr){
        CHECK_CUDA(hipFree(this->Axf_d));
        this->Axf_d = nullptr;
    }
...
template <typename T>
sparse_CSR_Matrix<T>::sparse_CSR_Matrix(int nx, int ny, int nz, local_int_t nnz, MatrixType mt, T* vals, local_int_t* row_ptr, local_int_t* col_idx) {
    ...
    this->row_ptr = std::vector<local_int_t>(row_ptr, row_ptr + this->num_rows + 1);
    this->col_idx = std::vector<local_int_t>(col_idx, col_idx + this->nnz);
...
template <typename T>
sparse_CSR_Matrix<T>::sparse_CSR_Matrix(int nx, int ny, int nz, local_int_t nnz, MatrixType mt, std::vector<T> vals, std::vector<local_int_t> row_ptr, std::vector<local_int_t> col_idx) {
...
sparse_CSR_Matrix<T>::sparse_CSR_Matrix(std::vector<std::vector<T>> dense_matrix){
    ...
    local_int_t num_rows = dense_matrix.size();
    local_int_t num_cols = dense_matrix[0].size();
    ...
    for (local_int_t i = 0; i< num_rows; i++){
        for (local_int_t j = 0; j < num_cols; j++){
...
void sparse_CSR_Matrix<T>::generateMatrix_onCPU(int nx, int ny, int nz){
    ...
    std::vector<std::vector<T>> values_per_row(num_rows);
    ...
    this->row_ptr = std::vector<local_int_t>(num_rows + 1, 0);
    this->col_idx = std::vector<local_int_t>();
...
void sparse_CSR_Matrix<T>::generateMatrix_onGPU(int nx, int ny, int nz)
{
    ...
    CHECK_CUDA(hipMalloc(&this->row_ptr_d, (num_nodes + 1) * sizeof(local_int_t)));
    CHECK_CUDA(hipMalloc(&this->col_idx_d, nnz * sizeof(local_int_t)));
    CHECK_CUDA(hipMalloc(&this->values_d, nnz * sizeof(T)));

    // set them to zero
    // std::cout << "row_ptr_d: " << row_ptr_d << std::endl;
    CHECK_CUDA(hipMemset(this->row_ptr_d, 0, (num_nodes + 1) * sizeof(local_int_t)));
    CHECK_CUDA(hipMemset(this->col_idx_d, 0, nnz * sizeof(local_int_t)));
    CHECK_CUDA(hipMemset(this->values_d, 0, nnz * sizeof(T)));
...
void sparse_CSR_Matrix<T>::sparse_CSR_Matrix_from_striped_transformation_CPU(striped_Matrix<T> & A){
    ...
    this->row_ptr = std::vector<local_int_t>(this->num_rows + 1, 0);
    this->col_idx = std::vector<local_int_t>(this->nnz, 0);
...
void sparse_CSR_Matrix<T>::sparse_CSR_Matrix_from_striped_transformation_GPU(striped_Matrix<T> & A){
    ...
    // allocate space for the device pointers
    CHECK_CUDA(hipMalloc(&this->row_ptr_d, (this->num_rows + 1) * sizeof(local_int_t)));
    CHECK_CUDA(hipMalloc(&this->col_idx_d, this->nnz * sizeof(local_int_t)));
    CHECK_CUDA(hipMalloc(&this->values_d, this->nnz * sizeof(T)));

    // set them to zero
    CHECK_CUDA(hipMemset(this->row_ptr_d, 0, (this->num_rows + 1) * sizeof(local_int_t)));
    CHECK_CUDA(hipMemset(this->col_idx_d, 0, this->nnz * sizeof(local_int_t)));
    CHECK_CUDA(hipMemset(this->values_d, 0, this->nnz * sizeof(T)));

    // std::cout << "this is called prior" << std::endl;
    // std::cout << "nnz: " << this->nnz << std::endl;
    local_int_t new_nnz = generate_CSR_from_Striped(
    ...
    if(A.get_f2c_op_d() != nullptr){
        CHECK_CUDA(hipMalloc(&this->f2c_op_d, this->num_rows * sizeof(local_int_t)));
        CHECK_CUDA(hipMemcpy(this->f2c_op_d, A.get_f2c_op_d(), this->num_rows * sizeof(local_int_t), hipMemcpyDeviceToDevice));
    }
    if(A.get_rc_d() != nullptr){
        CHECK_CUDA(hipMalloc(&this->rc_d, this->num_rows * sizeof(T)));
        CHECK_CUDA(hipMemcpy(this->rc_d, A.get_rc_d(), this->num_rows * sizeof(T), hipMemcpyDeviceToDevice));
    }
    if(A.get_xc_d() != nullptr){
        CHECK_CUDA(hipMalloc(&this->xc_d, this->num_rows * sizeof(T)));
        CHECK_CUDA(hipMemcpy(this->xc_d, A.get_xc_d(), this->num_rows * sizeof(T), hipMemcpyDeviceToDevice));
    }
    if(A.get_Axf_d() != nullptr){
        CHECK_CUDA(hipMalloc(&this->Axf_d, this->num_rows * sizeof(T)));
        CHECK_CUDA(hipMemcpy(this->Axf_d, A.get_Axf_d(), this->num_rows * sizeof(T), hipMemcpyDeviceToDevice));
    }
...
void sparse_CSR_Matrix<T>::iterative_values(){
    ...
    for (local_int_t i = 0; i < this->nnz; i++) {
        this->values[i] = val;
        val += 0.1;
        if (val > 10.0) {
            val = 0.1;
...
template <typename T>
std::vector<local_int_t>& sparse_CSR_Matrix<T>::get_row_ptr(){
...
template <typename T>
std::vector<local_int_t>& sparse_CSR_Matrix<T>::get_col_idx(){
    // these assume that the matrix is on the CPU, so if they are not, we need to copy them to the CPU
    if(this->row_ptr_d != nullptr or this->col_idx_d != nullptr or this->values_d != nullptr){
        this->copy_Matrix_toCPU();
    }
...
template <typename T>
local_int_t* sparse_CSR_Matrix<T>::get_row_ptr_d(){
...
template <typename T>
local_int_t* sparse_CSR_Matrix<T>::get_col_idx_d(){
    // these assume that the matrix is on the GPU, so if they are not, we need to copy them to the GPU
    // if(this->row_ptr_d == nullptr or this->col_idx_d == nullptr or this->values_d == nullptr){
    //     this->copy_Matrix_toGPU();
    // }
...
void sparse_CSR_Matrix<T>::copy_Matrix_toGPU(){
    ...
    CHECK_CUDA(hipMemcpy(this->row_ptr_d, this->row_ptr.data(), (this->num_rows + 1) * sizeof(local_int_t), hipMemcpyHostToDevice));
    CHECK_CUDA(hipMemcpy(this->col_idx_d, this->col_idx.data(), this->nnz * sizeof(local_int_t), hipMemcpyHostToDevice));
    CHECK_CUDA(hipMemcpy(this->values_d, this->values.data(), this->nnz * sizeof(T), hipMemcpyHostToDevice));
    
    // not every matrix has a f2c operator
    if(not this->f2c_op.empty()){
        CHECK_CUDA(hipMalloc(&this->f2c_op_d, this->num_rows * sizeof(local_int_t)));
        CHECK_CUDA(hipMemcpy(this->f2c_op_d, this->f2c_op.data(), this->num_rows * sizeof(local_int_t), hipMemcpyHostToDevice));
    }
...
void sparse_CSR_Matrix<T>::copy_Matrix_toCPU(){
    ...
    if(this->f2c_op_d != nullptr){
        ...
        CHECK_CUDA(hipMemcpy(this->f2c_op.data(), this->f2c_op_d, this->num_rows * sizeof(local_int_t), hipMemcpyDeviceToHost));
    }
    ...
    CHECK_CUDA(hipMemcpy(this->row_ptr.data(), this->row_ptr_d, (this->num_rows + 1) * sizeof(local_int_t), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(this->col_idx.data(), this->col_idx_d, this->nnz * sizeof(local_int_t), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(this->values.data(), this->values_d, this->nnz * sizeof(T), hipMemcpyDeviceToHost));
...
void sparse_CSR_Matrix<T>::generate_f2c_operator_onGPU(){
    ...
    // allocate space for the device pointers
    CHECK_CUDA(hipMalloc(&this->f2c_op_d, fine_n_rows * sizeof(local_int_t)));

    // set them to zero
    CHECK_CUDA(hipMemset(this->f2c_op_d, 0, fine_n_rows * sizeof(local_int_t)));
...
void sparse_CSR_Matrix<T>::generate_f2c_operator_onCPU(){
    ...
    this->f2c_op = std::vector<local_int_t>(nxf*nyf*nzf, 0);
...
void sparse_CSR_Matrix<T>::initialize_coarse_Matrix(){
    ...
    local_int_t nx_c = this->nx / 2;
    local_int_t ny_c = this->ny / 2;
    local_int_t nz_c = this->nz / 2;

    local_int_t nx_f = this->nx;
    local_int_t ny_f = this->ny;
    local_int_t nz_f = this->nz;
    ...
    if(this->row_ptr_d != nullptr and this->col_idx_d != nullptr and this->values_d != nullptr){
        ...
        CHECK_CUDA(hipMalloc(&this->coarse_Matrix->rc_d, coarse_num_rows * sizeof(T)));
        CHECK_CUDA(hipMalloc(&this->coarse_Matrix->xc_d, coarse_num_rows * sizeof(T)));
        CHECK_CUDA(hipMalloc(&this->coarse_Matrix->Axf_d, fine_num_rows* sizeof(T)));

        // set them to zero
        CHECK_CUDA(hipMemset(this->coarse_Matrix->rc_d, 0, coarse_num_rows * sizeof(T)));
        CHECK_CUDA(hipMemset(this->coarse_Matrix->xc_d, 0, coarse_num_rows * sizeof(T)));
        CHECK_CUDA(hipMemset(this->coarse_Matrix->Axf_d, 0, fine_num_rows * sizeof(T)));
...
template <typename T>
local_int_t sparse_CSR_Matrix<T>::get_num_rows() const{
    return this->num_rows;
}

template <typename T>
local_int_t sparse_CSR_Matrix<T>::get_num_cols() const{
...
template <typename T>
local_int_t sparse_CSR_Matrix<T>::get_nnz() const{
    return this->nnz;
}
...
template <typename T>
local_int_t* sparse_CSR_Matrix<T>::get_f2c_op_d(){
    return this->f2c_op_d;
}

template <typename T>
std::vector<local_int_t> sparse_CSR_Matrix<T>::get_f2c_op(){
    // std::cout << "from sparse_matrix size of f2c_op: " << this->f2c_op.size() << std::endl;
    return this->f2c_op;
}

template <typename T>
T sparse_CSR_Matrix<T>::get_element(local_int_t row, local_int_t col) const{
    // this currently only works if the matrix is on the CPU
    this->sanity_check_Matrix_on_CPU();
    
    local_int_t start = this->row_ptr[row];
    local_int_t end = this->row_ptr[row + 1];
...
void sparse_CSR_Matrix<T>::print() const{
    ...
    for (local_int_t i = 0; i < this->num_rows + 1; i++) {
...
bool sparse_CSR_Matrix<T>::compare_to(sparse_CSR_Matrix<T>& other, std::string info){
    ...
    if (same && this->development) {
        ...
        for (local_int_t i = 0; i < this->num_rows; i++) {
            local_int_t start = this->row_ptr[i];
            local_int_t end = this->row_ptr[i + 1];
            local_int_t other_start = other.get_row_ptr()[i];
    ...
    if(this->f2c_op.empty() and not other.get_f2c_op().empty() or
        ...
        for(local_int_t i = 0; i < other_f2c.size(); i++){

    ...
    )
{
    ...
    CHECK_CUDA(hipMemcpy(x_original.data(), x_d, A.get_num_rows() * sizeof(DataType), hipMemcpyDeviceToHost));
    ...
    if( // the MG preconditioner can only be applied for matrices divisible by 8 and the result needs to be bigger than 2
        ...
        for(int i = 0; i < num_iterations; i++){
            ...
            // restore original x
            CHECK_CUDA(hipMemcpy(x_d, x_original.data(), A.get_num_rows() * sizeof(DataType), hipMemcpyHostToDevice));
        }
    } else{
    ...
    )
{
    ...
    CHECK_CUDA(hipMemcpy(x_original.data(), x_d->x_d, size_halo * sizeof(DataType), hipMemcpyDeviceToHost));
    ...
    if( // the MG preconditioner can only be applied for matrices divisible by 8 and the result needs to be bigger than 2
        ...
        for(int i = 0; i < num_iterations; i++){
            ...
            // restore original x
            CHECK_CUDA(hipMemcpy(x_d->x_d, x_original.data(), size_halo * sizeof(DataType), hipMemcpyHostToDevice));
        }
    } else{
    ...
    )
{
    ...
    CHECK_CUDA(hipMemcpy(y.data(), y_d, A.get_num_rows() * sizeof(DataType), hipMemcpyDeviceToHost));
    ...
    CHECK_CUDA(hipMemcpy(y_d, y.data(), A.get_num_rows() * sizeof(DataType), hipMemcpyHostToDevice));
}
...
void bench_SPMV(
    ...
    CHECK_CUDA(hipMemcpy(y.data(), y_d->x_d, size_halo * sizeof(DataType), hipMemcpyDeviceToHost));
    ...
    // copy the original vector back
    CHECK_CUDA(hipMemcpy(y_d->x_d, y.data(), size_halo * sizeof(DataType), hipMemcpyHostToDevice));
}
...
void bench_WAXPBY(
    ...
    CHECK_CUDA(hipMemcpy(w.data(), w_d, A.get_num_rows() * sizeof(DataType), hipMemcpyDeviceToHost));
    ...
    if(implementation.test_before_bench){
        ...
        CHECK_CUDA(hipMemcpy(w_d, w.data(), A.get_num_rows() * sizeof(DataType), hipMemcpyHostToDevice));
...
void bench_WAXPBY(
    ...
    CHECK_CUDA(hipMemcpy(w.data(), w_d->x_d, size_halo * sizeof(DataType), hipMemcpyDeviceToHost));
    ...
    for(int i = 0; i < num_iterations; i++){
        ...
        // restore original value of w_d
        CHECK_CUDA(hipMemcpy(w_d->x_d, w.data(), size_halo * sizeof(DataType), hipMemcpyHostToDevice));
    }
}
    ...
    )
{
    ...
    CHECK_CUDA(hipMemcpy(x.data(), x_d, A.get_num_rows() * sizeof(DataType), hipMemcpyDeviceToHost));
    ...
    for(int i = 0; i < num_iterations; i++){
        ...
        CHECK_CUDA(hipMemcpy(x_d, x.data(), A.get_num_cols() * sizeof(DataType), hipMemcpyHostToDevice));
        timer.startTimer();
    ...
    // copy the original vector back
    CHECK_CUDA(hipMemcpy(x_d, x.data(), A.get_num_rows() * sizeof(DataType), hipMemcpyHostToDevice));
...
{   
    ...
    CHECK_CUDA(hipMemcpy(x.data(), x_d->x_d, size_halo * sizeof(DataType), hipMemcpyDeviceToHost));   
    ...
    for (int i = 0; i < num_iterations; i++){
        ...
        // std::cout<< "Num iterations: " << num_iterations << std::endl;
        // copy original x into x_d
        CHECK_CUDA(hipMemcpy(x_d->x_d, x.data(), size_halo * sizeof(DataType), hipMemcpyHostToDevice));
        timer.startTimer();
...
{   
    ...
    CHECK_CUDA(hipMemcpy(a_original.data(), a_d, num_sanity_elements * sizeof(DataType), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(b_original.data(), b_d, num_sanity_elements * sizeof(DataType), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(x_original.data(), x_d, num_sanity_elements * sizeof(DataType), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(y_original.data(), y_d, num_sanity_elements * sizeof(DataType), hipMemcpyDeviceToHost));

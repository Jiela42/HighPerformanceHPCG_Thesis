...
void test_SPMV(striped_multi_GPU_Implementation<DataType>& implementation_multi_GPU, striped_partial_Matrix<DataType>* A_local_striped, striped_Matrix<DataType>* A_global_striped, Halo *halo_p_d, Halo *halo_Ap_d, Problem *problem){
    ...
    if(problem->rank == 0){
        ...
        DataType *p_global_d;
        CHECK_CUDA(hipMalloc(&p_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType)));
        CHECK_CUDA(hipMemcpy(p_global_d, p_global_h, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyHostToDevice));
        ...
        CHECK_CUDA(hipMalloc(&Ap_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType)));
        CHECK_CUDA(hipMemcpy(Ap_global_d, Ap_global_h, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyHostToDevice));
        ...
        //copy result to CPU
        CHECK_CUDA(hipMemcpy(result_single_GPU_h, Ap_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyDeviceToHost));
        ...
        free(p_global_h);
        free(Ap_global_h);
        CHECK_CUDA(hipFree(p_global_d));
        CHECK_CUDA(hipFree(Ap_global_d));
    }else{
...
void test_SymGS(striped_multi_GPU_Implementation<DataType>& implementation_multi_GPU, striped_partial_Matrix<DataType>* A_local_striped, striped_Matrix<DataType>* A_global_striped, Halo *halo_p_d, Halo *halo_Ap_d, Problem *problem){
    ...
    if(problem->rank == 0){
        ...
        CHECK_CUDA(hipMemcpy(result_single_GPU_h, p_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyDeviceToHost));
    ...
    for(DataType alpha = -1.0; alpha <= 1.0; alpha += 0.5){
        for(DataType beta = -1.0; beta<= 1.0; beta += 0.5){
            ...
            if(problem->rank == 0){
                ...
                DataType *w_global_d;
                CHECK_CUDA(hipMalloc(&w_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType)));
                CHECK_CUDA(hipMemcpy(w_global_d, w_global_h, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyHostToDevice));
                ...
                DataType *x_global_d;
                CHECK_CUDA(hipMalloc(&x_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType)));
                CHECK_CUDA(hipMemcpy(x_global_d, x_global_h, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyHostToDevice));
                ...
                DataType *y_global_d;
                CHECK_CUDA(hipMalloc(&y_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType)));
                CHECK_CUDA(hipMemcpy(y_global_d, y_global_h, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyHostToDevice));
                ...
                CHECK_CUDA(hipMemcpy(result_single_GPU_h, w_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyDeviceToHost));
                ...
                free(x_global_h);
                free(y_global_h);
                CHECK_CUDA(hipFree(w_global_d));
                CHECK_CUDA(hipFree(x_global_d));
                CHECK_CUDA(hipFree(y_global_d));
...
void test_Dot(striped_multi_GPU_Implementation<DataType>& implementation_multi_GPU, striped_Matrix<DataType>* A_global_striped, Halo *halo_x_local_d, Halo *halo_y_local_d, Problem *problem){
    ...
    CHECK_CUDA(hipMemcpy(&result_multi_GPU_h, result_multi_GPU_d, sizeof(DataType), hipMemcpyDeviceToHost));
    ...
    if(problem->rank == 0){
        ...
        CHECK_CUDA(hipMalloc(&result_single_GPU_d, sizeof(DataType)));
        CHECK_CUDA(hipMemset(result_single_GPU_d, 0, sizeof(DataType)));
        ...
        CHECK_CUDA(hipMemcpy(&result_single_GPU_h, result_single_GPU_d, sizeof(DataType), hipMemcpyDeviceToHost));
        ...
        CHECK_CUDA(hipFree(x_global_d));
        CHECK_CUDA(hipFree(y_global_d));
        CHECK_CUDA(hipFree(result_single_GPU_d));
        CHECK_CUDA(hipFree(result_multi_GPU_d));
...
void test_CG(striped_multi_GPU_Implementation<DataType>& implementation_multi_GPU, striped_partial_Matrix<DataType>* A_local_striped, striped_Matrix<DataType>* A_global_striped, Halo *halo_b_local_d, Halo *halo_x_local_d, Problem *problem){
    ...
    if(problem->rank == 0){
        ...
        DataType *b_global_d;
        CHECK_CUDA(hipMalloc(&b_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType)));
        CHECK_CUDA(hipMemcpy(b_global_d, b_global_h, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyHostToDevice));
        ...
        //copy result to CPU
        CHECK_CUDA(hipMemcpy(result_single_GPU_h, x_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyDeviceToHost));
...
void test_MG(striped_multi_GPU_Implementation<DataType>& implementation_multi_GPU, striped_partial_Matrix<DataType>* A_local_striped, striped_Matrix<DataType>* A_global_striped, Halo *halo_r_local_d, Halo *halo_x_local_d, Problem *problem){
    ...
    if(problem->rank == 0){
        ...
        DataType *r_global_d;
        CHECK_CUDA(hipMalloc(&r_global_d, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType)));
        CHECK_CUDA(hipMemcpy(r_global_d, r_global_h, NPX*NX*NPY*NY*NPZ*NZ*sizeof(DataType), hipMemcpyHostToDevice));
 ...
 */
    ...
    /* CHECK_CUDA(hipMemcpy(x_c, A_part.get_coarse_Matrix()->get_values_d(), sizeof(DataType) * num_rows_local/8 * 27, hipMemcpyDeviceToHost));
    flag=1;

...
bool run_banded_warp_reduction_tests_on_matrix(sparse_CSR_Matrix<double> A){
    ...
    banded_warp_reduction_Implementation<double> banded_warp_reduction;
    ...
    all_pass = all_pass && test_Dot(
        ...
        A_banded.get_nx(), A_banded.get_ny(), A_banded.get_nz()
        );
...
bool run_bandedWarpReduction_tests(int nx, int ny, int nz){
    ...
    if(not all_pass){
        std::cout << "banded_warp_reduction tests failed for standard HPCG Matrix and size " << nx << "x" << ny << "x" << nz << std::endl;
    }

...
bool run_cuSparse_tests_on_Matrix(sparse_CSR_Matrix<DataType>& A){
    ...
    CHECK_CUDA(hipMalloc(&a_d, A.get_num_cols() * sizeof(DataType)));
    CHECK_CUDA(hipMalloc(&b_d, A.get_num_cols() * sizeof(DataType)));

    // copy the data to the device
    CHECK_CUDA(hipMemcpy(a_d, a.data(), A.get_num_cols() * sizeof(DataType), hipMemcpyHostToDevice));
    CHECK_CUDA(hipMemcpy(b_d, b.data(), A.get_num_cols() * sizeof(DataType), hipMemcpyHostToDevice));

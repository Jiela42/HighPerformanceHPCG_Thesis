...
bool run_banded_preprocessed_tests_on_matrix(sparse_CSR_Matrix<double> A){
    ...
    banded_preprocessed_Implementation<double> banded_preprocessed;
...
bool run_bandedPreprocessed_tests(int nx, int ny, int nz){
    ...
    all_pass = all_pass && run_banded_preprocessed_tests_on_matrix(A);

    if(not all_pass){
        std::cout << "banded_preprocessed tests failed for standard HPCG Matrix and size " << nx << "x" << ny << "x" << nz << std::endl;
    }

...
void print_rrNorm(int nx, int ny, int nz){
    ...
    CHECK_CUDA(hipMalloc(&x_d, x.size() * sizeof(double)));
    CHECK_CUDA(hipMalloc(&y_d, y.size() * sizeof(double)));

    CHECK_CUDA(hipMemcpy(x_d, x.data(), x.size() * sizeof(double), hipMemcpyHostToDevice));
    CHECK_CUDA(hipMemcpy(y_d, y.data(), y.size() * sizeof(double), hipMemcpyHostToDevice));
    ...
    if (COMPARE_NORMS){
        ...
        CHECK_CUDA(hipMemset(x_d, 0, x.size() * sizeof(double)));

...
bool parallel_CSR_generation_test(sparse_CSR_Matrix<double>& A_copy){
    ...
    CHECK_CUDA(hipMemcpy(row_ptr_host.data(), A_copy.get_row_ptr_d(), (nx*ny*nz + 1) * sizeof(local_int_t), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(col_idx_host.data(), A_copy.get_col_idx_d(), A_copy.get_nnz() * sizeof(local_int_t), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(values_host.data(), A_copy.get_values_d(), A_copy.get_nnz() * sizeof(double), hipMemcpyDeviceToHost));
...
bool parallel_generation_from_CSR_test(sparse_CSR_Matrix<double>& A){
    ...
    CHECK_CUDA(hipMemcpy(j_min_i_host.data(), striped_A->get_j_min_i_d(), striped_A->get_num_stripes() * sizeof(local_int_t), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(values_host.data(), striped_A->get_values_d(), striped_A->get_num_stripes() * nx * ny * nz * sizeof(double), hipMemcpyDeviceToHost));
...
bool box_coloring_test(striped_Matrix<double>& A){
    ...
    CHECK_CUDA(hipMemcpy(color_idx_ptr.data(), A.get_color_pointer_d(), 28 * sizeof(local_int_t), hipMemcpyDeviceToHost));
    CHECK_CUDA(hipMemcpy(color_sorted_rows.data(), A.get_color_sorted_rows_d(), A.get_num_rows() * sizeof(local_int_t), hipMemcpyDeviceToHost));
...
bool run_MG_data_tests(sparse_CSR_Matrix<double>& A){
    ...
    while(current_matrix->get_coarse_Matrix() != nullptr){
        ...
        CHECK_CUDA(hipMemcpy(c2f_device.data(), current_matrix->get_f2c_op_d(), fine_n_rows * sizeof(int), hipMemcpyDeviceToHost));

#ifndef STRIPED_COR_BOX_COLORING_CUH
#define STRIPED_COR_BOX_COLORING_CUH
...
template <typename T>
class striped_COR_box_coloring_Implementation : public striped_warp_reduction_Implementation<T> {
public:

    int bx, by, bz; // the box size in x, y and z direction
    int SymGS_cooperation_number; // the cooperation number for the SymGS

    striped_COR_box_coloring_Implementation(){

        // overwritting the inherited variables

        this->version_name = "Striped Box coloring (COR stored on GPU)";
    ...
    void compute_SymGS(
        ...
        striped_COR_box_coloring_computeSymGS(A, x_d, y_d);
    }

private:

    void striped_COR_box_coloring_computeSymGS(
        striped_Matrix<T> & A,
        T * x_d, T * y_d // the vectors x and y are already on the device
    );
};
...
#endif // STRIPED_COR_BOX_COLORING_CUH
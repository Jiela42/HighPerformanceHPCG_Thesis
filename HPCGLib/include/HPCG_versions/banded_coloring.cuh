#ifndef BANDED_coloring_CUH
#define BANDED_coloring_CUH
...
template <typename T>
class banded_coloring_Implementation : public HPCG_functions<T> {
public:

    banded_coloring_Implementation(){

        // overwritting the inherited variables

        this->version_name = "Banded coloring";
    ...
    void compute_CG(sparse_CSR_Matrix<T>& A, std::vector<T>& b, std::vector<T>& x) override {
        std::cerr << "Warning: compute_CG is not implemented in Banded coloring." << std::endl;
    }
    ...
    void compute_MG(
        ...
        std::cerr << "Warning: compute_MG is not implemented in Banded coloring." << std::endl;
    }
    ...
    void compute_SymGS(
        ...
        std::cerr << "Warning: compute_SymGS requires different arguments in Banded coloring." << std::endl;
    }
    ...
    void compute_SymGS(
        ...
        banded_coloring_computeSymGS(A, banded_A_d, num_rows, num_cols, num_bands, j_min_i, x_d, y_d);
    }
    ...
    void compute_SPMV(
        ...
        throw std::runtime_error("ERROR: compute_SPMV requires a banded Matrix as input in the Banded coloring Implementation.");
    }
    ...
    void compute_WAXPBY(
        ...
        std::cerr << "Warning: compute_WAXPBY is not implemented in Banded coloring." << std::endl;
    }
    ...
    void compute_Dot(
        ...
        std::cerr << "Warning: compute_Dot is not implemented using the sparse_CSR Matrix in Banded coloring." << std::endl;
    }
    ...
    void compute_Dot(
        ...
        std::cerr << "Warning: compute_Dot is not implemented using the banded Matrix in Banded coloring." << std::endl;
    }
    ...
    void compute_SPMV(
        ...
        std::cerr << "Warning: compute_SPMV requires different arguments in Banded coloring." << std::endl;
    }

private:

    void banded_coloring_computeSymGS(
        banded_Matrix<T> & A, // we pass A for the metadata
        T * banded_A_d, // the data of matrix A is already on the device
        int num_rows, int num_cols,
        int num_bands, // the number of bands in the banded matrix

...
class HPCG_functions {
    ...
    double L2_norm_for_SymGS(
        ...
        double *Ax;
        double *result;
        CHECK_CUDA(hipMalloc(&Ax, num_rows * sizeof(T)));
        CHECK_CUDA(hipMalloc(&result, 1 * sizeof(T)));
    
        CHECK_CUDA(hipMemset(Ax, 0, num_rows * sizeof(T)));
        CHECK_CUDA(hipMemset(result, 0, 1 * sizeof(T)));
        ...
        // copy result over
        T result_h;
        CHECK_CUDA(hipMemcpy(&result_h, result, 1 * sizeof(T), hipMemcpyDeviceToHost));

...
void ReportResults(const SparseMatrix & A, int numberOfMgLevels, int numberOfCgSets, int refMaxIters,int optMaxIters, double times[],
  ...
  if (A.geom->rank==0) { // Only PE 0 needs to compute and report timing results
    ...
    doc.get("Global Problem Dimensions")->add("Global nx",static_cast<long long>(A.geom->gnx));
    doc.get("Global Problem Dimensions")->add("Global ny",static_cast<long long>(A.geom->gny));
    doc.get("Global Problem Dimensions")->add("Global nz",static_cast<long long>(A.geom->gnz));

    doc.add("Processor Dimensions","");
    doc.get("Processor Dimensions")->add("npx",static_cast<long long>(A.geom->npx));
    doc.get("Processor Dimensions")->add("npy",static_cast<long long>(A.geom->npy));
    doc.get("Processor Dimensions")->add("npz",static_cast<long long>(A.geom->npz));

    doc.add("Local Domain Dimensions","");
    doc.get("Local Domain Dimensions")->add("nx",static_cast<long long>(A.geom->nx));
    doc.get("Local Domain Dimensions")->add("ny",static_cast<long long>(A.geom->ny));
    ...
    for (int i=0; i< A.geom->npartz; ++i) {
      ...
      doc.get("Local Domain Dimensions")->add("nz",static_cast<long long>(A.geom->partz_nz[i]));
    ...
    doc.get("Linear System Information")->add("Number of Equations",static_cast<long long>(A.totalNumberOfRows));
    doc.get("Linear System Information")->add("Number of Nonzero Terms",static_cast<long long>(A.totalNumberOfNonzeros));
    ...
    for (int i=1; i<numberOfMgLevels; ++i) {
      ...
      doc.get("Multigrid Information")->get("Coarse Grids")->add("Number of Equations",static_cast<long long>(Af->Ac->totalNumberOfRows));
      doc.get("Multigrid Information")->get("Coarse Grids")->add("Number of Nonzero Terms",static_cast<long long>(Af->Ac->totalNumberOfNonzeros));

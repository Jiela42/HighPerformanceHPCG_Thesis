...
void replaceStripedMatrixDiagonal(striped_partial_Matrix<DataType>& A_striped, int rank){
  DataType *values_d = A_striped.get_values_d();
  DataType *values_h = (DataType*)malloc(A_striped.get_num_rows() * A_striped.get_num_stripes() * sizeof(DataType)); 
  cudaMemcpy(values_h, values_d, A_striped.get_num_rows() * A_striped.get_num_stripes() * sizeof(DataType), cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
  int stripes = A_striped.get_num_stripes();
  int rows = A_striped.get_num_rows();
  for(local_int_t i = 0; i< rows; i++){
    if(rank == 0 && i<9){
      values_h[i * stripes + 13] = (i+2)*1.0e6;
    }else{
      values_h[i * stripes + 13] = 1.0e6;
    }
  }
  cudaMemcpy(values_d, values_h, A_striped.get_num_rows() * A_striped.get_num_stripes() * sizeof(DataType), cudaMemcpyHostToDevice);
  cudaDeviceSynchronize();
  free(values_h);
}

void restoreStripedMatrixDiagonal(striped_partial_Matrix<DataType>& A_striped, int rank){
  DataType *values_d = A_striped.get_values_d();
  DataType *values_h = (DataType*)malloc(A_striped.get_num_rows() * A_striped.get_num_stripes() * sizeof(DataType)); 
  cudaMemcpy(values_h, values_d, A_striped.get_num_rows() * A_striped.get_num_stripes() * sizeof(DataType), cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
...
int TestCG(SparseMatrix & A, CGData & data, Vector & b, Vector & x, TestCGData & testcg_data) {
  ...
  replaceStripedMatrixDiagonal(*(A.A_striped), A.problem->rank);
  ...
  restoreStripedMatrixDiagonal(*(A.A_striped), A.problem->rank);
  CopyVector(origB, b);
  // Delete vectors
